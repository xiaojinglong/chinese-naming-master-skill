...
| 工具 | 用途 |
|------|------|
| `_tools/data_check.py` | 数据完整性检查器：启动时校验 11 项数据源，路径错误不再静默（V3.2 静默失效 bug 的根治） |
| `benchmark/run_benchmark.py` | 基准回归：固化 20 个已知好名/坏名/日柱用例，算法改动后自动跑分 |
| `tests/` | pytest 单元测试（11 项：数据完整性/评分/八字），`pytest tests/` 一键运行 |
| `_tools/compare_names.py` | 名字对比模式：两两名字多维度对比表 + 风格化推荐 |
| `.github/workflows/ci.yml` | GitHub Actions CI：push 自动跑数据检查 + 基准 + 单测 |

```bash
# 一键自检
python _tools/data_check.py        # 数据完整性
python benchmark/run_benchmark.py  # 基准回归

# -*- coding: utf-8 -*-
"""既明族定向深挖定稿：既X / X明 / 明X 变体，八字优先计分（五格仅参考）"""
import sys, os
...
# 78 个变体穷举 + 人工甄别（剔除老气/热字/谐音雷）后的定稿名单
FINAL_NAMES = ['陈雨明', '陈澈明', '陈明澈', '陈既洲', '陈既泽',
               '陈既清', '陈既平', '陈既明', '陈可明', '陈以明']
...
result = {
    'input': {'surname': '陈', 'gender': 'male',
              ...
              'requirement': '既明族定向深挖：沿用用户认可的「陈既明」句式，穷举 既X / 明X / X明 三种变体共 78 个（水木土安全区，忌金、火旺避火），人工甄别后定稿。双水组（雨明/澈明/明澈）八字契合满分；既字系水生木帮身；五格仅作参考不设门槛；重名率极低'},
    'bazi': bazi.to_dict(),
    'xiyongshen': xiyong, 'jiyongshen': jiyong, 'zodiac': zodiac,
    'candidates': final,
}
out_path = os.path.join(BASE, 'batch_reports', 'chen_jiming_family_report.html')
result['report_path'] = generate_html_report(result, output_path=out_path)

print('=== 既明族深挖定稿（八字优先计分） ===')

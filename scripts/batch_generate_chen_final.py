# -*- coding: utf-8 -*-
"""
陈姓高质量取名脚本 Final - 经典文学优先版
====================================
核心策略：优先使用经典文学中的双字词，每个时辰选择不同的名字
"""
import sys
import os
import json
...
from scoring_engine import load_data, score_name, _load_literature_data
...
def get_classic_names_from_literature():
    """从经典文学中提取适合取名的双字词"""
    literature = _load_literature_data()
...
def generate_names_for_shichen(surname, gender, year, month, day, hour, classic_pool, used_names, top_n=10):
    """为特定时辰生成名字，确保不重复"""
    # 获取八字信息
    ...
    literature_map = _load_literature_data()

    # 从经典池中筛选适合当前八字的名字
    scored_names = []
    for item in classic_pool:
        name = item['name']
...
# 获取经典名字池
classic_pool = get_classic_names_from_literature()
print(f'经典文学双字词池: {len(classic_pool)}个')
print()

# 存储所有结果
all_results = []
total_names = 0
used_names = set()  # 全局已使用的名字
...
for year, month, day in DATES:
    ...
    for shichen_name, hour in SHICHEN:
        # 生成名字
        names, bazi_info = generate_names_for_shichen(
            SURNAME, GENDER, year, month, day, hour, classic_pool, used_names, top_n=TOP_N_PER_SHICHEN
        )
...
# 保存HTML报告
report_path = os.path.join(output_dir, 'chen_classic_report_final.html')
...
# 保存JSON数据
json_path = os.path.join(output_dir, 'chen_classic_data_final.json')
